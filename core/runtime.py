"""Agent Runtime — ReAct 循环 + Hook 系统 + Scratchpad"""
...
class Runtime:
    ...
    def run(self, user_input: str) -> str:
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            # 构建 system prompt：指令 + 工具 + scratchpad + observation
            system = build_system_prompt(
                tools_schema=self.tools.render(),
                scratchpad=sp.render(),
                observation=observation,
            )

            # messages = system prompt + 对话历史（只含 user/assistant）
            messages = [{"role": "system", "content": system}]
            messages.extend(self.memory.get_history(20))
            ...
            if parsed is None:
                sp.add(step, "error", f"JSON 解析失败: {raw[:200]}")
                observation = f"[系统] JSON 解析失败，请重新输出正确的 JSON。原文: {raw[:200]}"
            ...
            if rtype == "tool_call":
                ...
                if tool is None:
                    observation = f"[错误] 工具 '{tool_name}' 不存在。可用: {', '.join(self.tools.list_tools())}"
                    sp.add(step, "action", f"{tool_name}({tool_args})")
                    sp.add(step, "error", observation)
                    self.hooks.on_tool_call_end(tool_name, False, observation[:50])
                    continue

                missing = [p for p in tool.get_required_params()
                           if p not in tool_args or not tool_args[p]]
                if missing:
                    observation = f"[错误] 工具 '{tool_name}' 缺少必填参数: {', '.join(missing)}"
                    sp.add(step, "action", f"{tool_name}({tool_args})")
                    sp.add(step, "error", observation)
                    self.hooks.on_tool_call_end(tool_name, False, observation[:50])
                    continue
                ...
                sp.add(step, "action", f"{tool_name}({json.dumps(tool_args, ensure_ascii=False)})")
                sp.add(step, "observation", observation)

                success = not observation.startswith("[")
                self.hooks.on_tool_call_end(tool_name, success, observation[:50])
                continue

            # ── 未知类型 ──
            sp.add(step, "error", f"未知响应类型: {rtype}")
            observation = f"[系统] 未知响应类型: {rtype}，请输出 final_answer 或 tool_call"
